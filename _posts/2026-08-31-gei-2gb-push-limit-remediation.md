---
title: GEI Migration Failure — 2GB Pack Size Limit Remediation Runbook
date: 2026-08-31 20:00:00 -0400
categories: [Azure DevOps, GitHub]
tags: [azure-devops, github-migrations, gei, git]
description: A step-by-step runbook for identifying and resolving GitHub Enterprise Importer migration failures caused by the 2GB pack-size push limit.
---

## Overview
This runbook helps identify and work toward mitigating migration failures caused by GitHub's 2GB pack-size limit during ADO → GitHub Enterprise Cloud migrations using GEI (`gh ado2gh migrate-repo`). This approach has been validated in a controlled test scenario — results may vary depending on repository structure and history, so validate on a backup copy before applying to production (see Step 1).

GitHub's own documentation covers this limit: [Troubleshooting the 2 GiB push limit](https://docs.github.com/en/get-started/using-git/troubleshooting-the-2-gb-push-limit). It describes two distinct failure patterns, and the right fix depends on which one applies:

- **Scenario A — many small commits, large cumulative history.** No single commit is oversized, but the total data being pushed in one operation (e.g. a first-time migration push of the entire repo) exceeds 2GB once packed. GitHub's documented fix here is to break the push into **stages** — push up to a sampled checkpoint commit, then the next, and so on — without rewriting any history. This scenario and its remediation are covered directly in GitHub's documentation; refer there for the staged-push steps.

- **Scenario B — a single commit itself exceeds 2GB.** The bulk of the oversized data is concentrated in one commit. GitHub's documentation notes that in this case, staged pushing won't help, and an interactive rebase to split that commit into smaller ones is required instead.

**This runbook focuses on Scenario B**, since that's what was confirmed through test reproduction — the `chunked-upload/{COMMIT_SHA}` reference in the migration log consistently pointed to the same single commit responsible for the oversized data, even after later commits were added on top. If your situation instead matches Scenario A (broad history, no single oversized commit), follow GitHub's staged-push guidance in the link above rather than the steps below.

**Scenario C — the large file doesn't need to be kept in history at all.** If the oversized files aren't actually needed going forward (e.g. old build outputs or binaries that shouldn't have been committed in the first place), removing them from history entirely is a simpler fix than splitting the commit. GitHub's documentation covers this option: [Removing files from a repository's history](https://docs.github.com/en/repositories/working-with-files/managing-large-files/about-large-files-on-github#removing-files-from-a-repositorys-history), via either `git filter-repo` or an interactive rebase (with the same caveats about the commit needing to belong to a single, non-merged branch). This is destructive to file content, not just commit SHAs, so only use it if the file genuinely doesn't need to be preserved.
...
### Optional: view the commit in the ADO web UI
Before jumping into the CLI, you can also inspect the commit directly in the browser by building the commit URL from the SHA:

```
https://dev.azure.com/{org}/{project}/_git/{repo}/commit/{COMMIT_SHA}
```

(or, on the visualstudio.com domain: `https://{org}.visualstudio.com/{project}/_git/{repo}/commit/{COMMIT_SHA}`)

This shows the commit's changed files and message in the UI — useful for a quick sanity check on what the commit contains before running the size-verification commands below, though it won't give exact byte sizes for binary files the way the CLI does.
